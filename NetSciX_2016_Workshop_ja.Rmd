...
ノードの中心性とグラフ全体における集権化についてです。前者は`res`で返され、後者は`centralization`や`theoretical_max`によって出力されます。中心性は`vids`パラメータを用いて作成したノードのサブセットでも使えます。全ての中心性を計算すると、リソースや時間を浪費してしまいそうな大規模なグラフで有用です。
...
分割に対してモジュラリティが高いということは、コミュニティ内のつながりが密であり、コミュニティ間のつながりが疎であることを反映している。
